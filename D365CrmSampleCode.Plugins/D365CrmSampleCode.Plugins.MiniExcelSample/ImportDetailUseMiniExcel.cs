...
namespace D365CrmSampleCode.Plugins.MiniExcelSample
{
    public class ImportDetailUseMiniExcel : IPlugin
    {
        #region Secure/Unsecure Configuration Setup
        private string _secureConfig = null;
        private string _unsecureConfig = null;

        public ImportDetailUseMiniExcel(string unsecureConfig, string secureConfig)
        {
            _secureConfig = secureConfig;
            _unsecureConfig = unsecureConfig;
        }
        #endregion
        public ImportDetailUseMiniExcel()
        {
        ...
        private void ImportInvoiceDetail(string base64Excel, string recordGuid, string entityName, IOrganizationService service, ref string state, ref string msg)
        {
            ...
            using (MemoryStream stream = new MemoryStream(excelBytes))
            {
                ...
                if (invoices.Count() > 0)
                {
                    ...
                    {
                        //Console.WriteLine($"Date: {invoice.Date}, Type: {invoice.Type}, Amount: {invoice.Amount}, Remark: {invoice.Remark}");
    ...
    /// <summary>
    /// Invoice dateil import template
    /// </summary>
